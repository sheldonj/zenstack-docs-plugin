...
function renderHeader(props: ViewPageProps): string[] {
  const documentMeta = extractDocumentMeta(props.view.attributes);
  const isDeprecated = Boolean(documentMeta.deprecated);
  const nameDisplay = isDeprecated ? `~~${props.view.name}~~` : props.view.name;
  const badges = isDeprecated
    ? ' <kbd>View</kbd> <kbd>Deprecated</kbd>'
    : ' <kbd>View</kbd>';
...
function renderMetadataBlock(props: ViewPageProps): string[] {
  ...
  return [
    ...renderMetadata(documentMeta, sourcePath),
