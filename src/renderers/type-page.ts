...
import { type DataModel, type TypeDef } from '@zenstackhq/language/ast';
...
export function renderTypePage(props: TypePageProps): string {
  const sortedFields = orderFields(
    props.typeDef.fields,
    props.options.fieldOrder,
  );
...
function renderUsedBySection(typeDef: TypeDef, usedBy: DataModel[]): string[] {
  ...
  for (const m of usedBy) {
    ...
    const fieldLinks = typeDef.fields
      ...
      .join(', ');
    lines.push(
      `- [${m.name}](../${dir}/${m.name}.md${anchor}) — ${fieldLinks}`,
    );
  }
